...
def shortest_path_incampus_method1():
        ...
        if node_end not in path_get:
            ...
            for node in path_get:
                if node in adj_list[min_index]:
                    if node.position.x == campus[student.which_campus].node[min_index].position.x:
                        dis = math.fabs(node.position.y - campus[student.which_campus].node[min_index].position.y)
                    else:
                        dis = math.fabs(node.position.x - campus[student.which_campus].node[min_index].position.x)
                    if dist[node.number] + dis == dist[min_index]:
                        pre_path[min_index].append(node)
            '''改变dist列表'''
            for element in adj_list[min_index]:
                if element not in path_get:
                    if element.position.x == campus[student.which_campus].node[min_index].position.x:
                        bi_dist = math.fabs(element.position.y - campus[student.which_campus].node[min_index].position.y)
                    else:
                        bi_dist = math.fabs(element.position.x - campus[student.which_campus].node[min_index].position.x)
